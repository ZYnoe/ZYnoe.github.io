    ...
    lx.data.ExampleData(
        ...
        extractions=[
            ...
            lx.data.Extraction(
                ...
                attributes={"abbreviation": "XRD", "purpose": "phase investigation"},
            ),
            lx.data.Extraction(
                extraction_class="characterization_technique",
                extraction_text="transmission electron microscopy",
                attributes={"abbreviation": "TEM", "purpose": "morphology"},
            ),
            lx.data.Extraction(
...
with open("abstracts.json", "r", encoding="utf-8") as f:
    ...
    # extract abstract
    abstracts = [item["abstract"].strip() for item in data if item.get("abstract")]
...
with ThreadPoolExecutor(max_workers=4) as executor:
    futures = {
        executor.submit(process_abstract, abstract): i
        for i, abstract in enumerate(demo_abstracts)
    }
    for future in as_completed(futures):
        result = future.result()
        results.append(result)
        print(f"Processed abstract {futures[future] + 1}/{len(demo_abstracts)}")


# Save the results to a JSONL file
lx.io.save_annotated_documents(
    results, output_name="extraction_results.jsonl", output_dir="."
)
