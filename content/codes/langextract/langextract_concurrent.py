...
def process_abstract(abstract, model_instance):  
    """Process a single abstract using the provided model instance."""  
    abstract = abstract.replace('\n', ' ')  
    return lx.extract(  
        text_or_documents=abstract,  
        prompt_description=prompt,  
        examples=examples,  
        model=model_instance,  # Use the model instance directly  
        fence_output=False,  
        use_schema_constraints=False,  
        max_char_buffer=1000,  # Larger chunks for efficiency  
    )  
  
# Create multiple Ollama model instances for parallel processing  
num_workers = 2  # Adjust based on your CPU/GPU resources  
models = [  
    OllamaLanguageModel(  
        model_id="gemma3:12b",  
...
with ThreadPoolExecutor(max_workers=num_workers) as executor:  
    # Submit all tasks  
    future_to_abstract = {  
        executor.submit(process_abstract, abstract, models[i % num_workers]): abstract  
        for i, abstract in enumerate(demo_abstracts)  
    }  
      
    # Collect results as they complete  
    for future in as_completed(future_to_abstract):  
        try:  
            result = future.result()  
            results.append(result)  
            print(f"Completed abstract {len(results)}/{len(demo_abstracts)}")  
        except Exception as e:  
            print(f"Error processing abstract: {e}")  
            # Optionally, you could retry here or add the error to a list  
